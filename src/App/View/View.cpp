...
QLayout* View::createMazeGenerateLayout() {
  ...
  generateBtn->setStyleSheet(
      "QPushButton { background-color:  #1c1a1a; color: #fcfcfc; font-size: 35px;} \
  ...
  QObject::connect(generateBtn, &QPushButton::pressed, this,
                   [this, mazeRowsTextEdit, mazeColsTextEdit]() {
...
QPushButton* View::createMazeLoadButton() {
  ...
  QObject::connect(loadBtn, &QPushButton::pressed, this, [this]() {
    ...
    if (!result.IsSuccess()) showErrorMessage(result.getErrorMessage().c_str());
  });
...
QPushButton* View::createGenCaveButton(QTextEdit* Rows, QTextEdit* Columns,
  ...
  genCaveBtn->setStyleSheet(
      "QPushButton { background-color: #1c1a1a; color: #fcfcfc; font-size: 35px;} \
                            QPushButton::hover { background-color: #2e2b2b; border: 2px solid #fafcfb; }");
  genCaveBtn->setMinimumSize(320, 35);

  QObject::connect(genCaveBtn, &QPushButton::pressed, this,
                   [this, Rows, Columns, initChance]() {
...
QLayout* View::createCaveLayout() {
  ...
  initChanceTextEdit->setStyleSheet(
      "border: 2px solid black; font-size: 17px;");
  ...
  QPushButton* genCaveBtn = createGenCaveButton(
      caveRowsTextEdit, caveColsTextEdit, initChanceTextEdit);

  QPushButton* loadCaveBtn = new QPushButton("LOAD");
  loadCaveBtn->setStyleSheet(
      "QPushButton { background-color: #1c1a1a; color: #fcfcfc; font-size: 35px;} \
                            QPushButton::hover { background-color: #2e2b2b; border: 2px solid #fafcfb; }");
  ...
  QObject::connect(loadCaveBtn, &QPushButton::pressed, this, [this]() {
    ...
    if (!res.IsSuccess()) showErrorMessage(res.getErrorMessage().c_str());
  });
  ...
  birthLimitTextEdit->setStyleSheet(
      "border: 2px solid black; font-size: 17px;");
  ...
  deathLimitTextEdit->setStyleSheet(
      "border: 2px solid black; font-size: 17px;");
  ...
  QPushButton* transOneStepCaveBtn =
      createCaveOneStepButton(birthLimitTextEdit, deathLimitTextEdit);
  ...
  QPushButton* transCaveAutoBtn = createCaveAutoButton(
      birthLimitTextEdit, deathLimitTextEdit, timeTextEdit);
  //##########################################################################3
  QVBoxLayout* vBox = new QVBoxLayout;
 ...
 * @return pointer to button
 */
QPushButton* View::createCaveOneStepButton(QTextEdit* BirthLimit,
                                           QTextEdit* DeathLimit) {
  QPushButton* oneStepCaveBtn = new QPushButton("ONE STEP");
  oneStepCaveBtn->setStyleSheet(
      "QPushButton { background-color: #1c1a1a; color: #fcfcfc; font-size: 35px;} \
                            QPushButton::hover { background-color: #2e2b2b; border: 2px solid #fafcfb; }");
  oneStepCaveBtn->setMinimumSize(320, 35);

  QObject::connect(oneStepCaveBtn, &QPushButton::pressed, this,
                   [this, BirthLimit, DeathLimit]() {
                     int birthLimit = BirthLimit->toPlainText().toInt();
                     int deathLimit = DeathLimit->toPlainText().toInt();

                     OpResult res =
                         model_.transformCave(false, birthLimit, deathLimit, 0);
 ...
 * @return pointer to button
 */
QPushButton* View::createCaveAutoButton(QTextEdit* BirthLimit,
                                        QTextEdit* DeathLimit,
                                        QTextEdit* timeEdit) {
  ...
  QObject::connect(oneStepCaveBtn, &QPushButton::pressed, this,
                   [this, BirthLimit, DeathLimit, timeEdit]() {
                     int birthLimit = BirthLimit->toPlainText().toInt();
                     int deathLimit = DeathLimit->toPlainText().toInt();
                     int time = timeEdit->toPlainText().toInt();

                     OpResult res = model_.transformCave(true, birthLimit,
                                                         deathLimit, time);
...
QLayout* View::createFindPathLayout() {
  ...
  QObject::connect(clearBtn, &QPushButton::pressed, this,
  ...
  findPathBtn->setStyleSheet(
      "QPushButton { background-color: #1c1a1a; color: #fcfcfc; font-size: 35px;} \
                            QPushButton::hover { background-color: #2e2b2b; border: 2px solid #fafcfb; }");
  findPathBtn->setMinimumSize(320, 35);

  QObject::connect(
      findPathBtn, &QPushButton::pressed, this,
      [this, startRowEdit, startColEdit, endRowEdit, endColEdit]() {
  ...
  QLabel* infoLabel = new QLabel(
      ...
      "<span style='color: white; background-color: black;'>CLEAR</span> - "
      "clear the path<br>"
      "To find a path through <span style='color: white; background-color: "
      "#1077de;'>Machine Learning</span>,<br>"
      "you need to <font color='red'>enter the coordinates of<br>"
      "the points</font> and click the <span style='color: white; "
      "background-color: black;'>FIND PATH</span><br><br>"
      "To use <span style='color: white; background-color: "
      "#1077de;'>conventional pathfinding tools</span>,<br>"
      "simply <font color='red'>left-click</font> to select a <font "
      "color='red'>starting</font> point<br>"
      "and <font color='blue'>right-click</font> to select a <font "
      "color='blue'>destination</font>");
